...
def getURL(mydb, url):
    ...
    mycursor.execute("SELECT fullname FROM teacher_neus")
...
def get_URL(link, id):
    ...
    for link in links:
        ...
        if (email == 'Verified email at neu.edu.vn') :
...
def saveSpecializedSoict(link, id) :
    ...
    for specialized in specializeds:
        ...
        record = cur.fetchall()
        if (len(record) != 0):
            sql = "INSERT INTO neu_specializeds (teacher_neu_id, specialized_id) VALUES (%s, %s)"
            val = [id, record[0][0]]
            cur.execute(sql, val)
        mydb.commit()
...
def saveTitle(link, id):
    ...
    if (id > 0) :
        ...
        record = cur.fetchall()
        if(len(record) == 0):
            sql = "INSERT INTO neu_titles (name, description, link, teacher_neu_id) VALUES (%s, %s, %s, %s)"
