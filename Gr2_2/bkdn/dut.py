...
def get_URL(mydb):
    url = 'http://dut.udn.vn/KhoaCNTT/Gioithieu/id/1464'
...
def saveData(val, mydb):
    cur = mydb.cursor()
    sql = "INSERT INTO teachers (fullname, position, email, degree, introduce, university, link) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    cur.execute(sql, val)
    mydb.commit()
...
def getContent(link, id):
    ...
    cur = mydb.cursor()
    val = [name, id, link, description, "dut"]
    sql = "INSERT INTO dut_titles (name, teacher_dut_id, link, description, university) VALUES (%s, %s, %s, %s, %s)"
    cur.execute(sql, val)
    mydb.commit()
